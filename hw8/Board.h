...
public:
	...
	*/
	int executePlaceMove(const PlaceMove & m);
	...
	//check if the move will cause any errors
	bool isLegal(const PlaceMove& m, Dictionary* dictionary, std::vector< std::pair<std::string, unsigned int> > wordsformed);

	bool moveHasStart(const PlaceMove& m);

	bool checkForAdjacentTiles(const PlaceMove& m);

	void init(std::string initfile);

	int firstCharacter(int col, int row, bool isHorizontal);

	std::string getWord(int col, int row, bool isHorizontal);

	bool canPlacePrefix(Dictionary* dictionary, std::string prefix, int col, int row, bool isHorizontal);
	bool checkPerpendicular(Dictionary* dictionary, char letter, int col, int row, bool isHorizontal);

	std::vector<Tile*>& getTilesToPlace();
	int pointsEarned(PlaceMove& m);
