...
def cmake_problem(problem):
	...
	# Find tests' output XML file, buiried a couple of directories deep
...
""" % test_name)
			...
			if match_list is None or len(match_list) < 1:
				# program may have died before it got to even producing the valgrind output
				pass
			...
			if type(test_results_element['Results']['NamedMeasurement']) != list:
				# if the test did not run because the excutable was not found, there is not the usual set of NamedMeasurements
				didnt_run = True
