...
std::vector<Tile*> PlaceMove::tileVector() const {
    ...
    for (unsigned int i = 0; i < tilescopy.length(); i++) {
        ...
        if (tilescopy.at(i) == '?') {
            tilescopy.erase(i, 1);
            //note: tileVector() allocates new memory to each tile pointer
            //have to properly deallocate every time tileVector() is called
            Tile* tile = new Tile(tilescopy.at(i), 0);
        ...
        for (unsigned int j = 0; j < handcopy.size(); j++) {
            if (tilescopy.at(i) == handcopy[j] -> getLetter()) {
                tilestoplace.push_back(handcopy[j]);
                break;
            }
        }
    }
