...
template<typename Key, typename Value>
testing::AssertionResult checkStructureRecursive(Node<Key, Value> * expectedNode, Node<Key, Value> * actualNode);
template<typename Key, typename Value>
...
testing::AssertionResult checkSameStructure(BinarySearchTree<Key, Value> & expected, BinarySearchTree<Key, Value> & actual)
{
	if(expected.mRoot == nullptr)
	{
		...
		{
			return testing::AssertionFailure() << "Node mismatch against expected tree! Tree is supposed to be empty, but instead it has root (" << actual.mRoot->getItem().first << ", " << actual.mRoot->getItem().second << ")";
		}
	}
	else
	{
		if(actual.mRoot == nullptr)
		{
			return testing::AssertionFailure() << "Node mismatch against expected tree! Tree is supposed to have root (" << expected.mRoot->getItem().first << ", " << expected.mRoot->getItem().second << "), but instead it is empty!";
		}
		else
		{
			// recursively compare rest of tree
			testing::AssertionResult recursiveResult = checkStructureRecursive(expected.mRoot, actual.mRoot);
...
template<typename Key, typename Value>
std::pair<int, bool> verifyBalanceRecursive(BinarySearchTree<Key, Value> & tree, Node<Key, Value>* currNode)
{
	if (currNode == nullptr)
	{
...
testing::AssertionResult checkBalance(BinarySearchTree<Key, Value> & tree)
{
	...
	{
		bool isActuallyBalanced = verifyBalanceRecursive(tree, tree.mRoot).second;
