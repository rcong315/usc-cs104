...
testing::AssertionResult checkHeightsHelper(AVLTree<Key, Value> & tree, AVLNode<Key, Value>* currRoot);
...
{
	return checkHeightsHelper(tree, dynamic_cast<AVLNode<Key, Value>*>(tree.mRoot));
}
...
{
	return verifyAVLBalanceRecursive(tree, dynamic_cast<AVLNode<Key, Value>*>(tree.mRoot)).second;
}
